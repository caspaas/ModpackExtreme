...
#define SUNLIGHTAMOUNT 1.0				//change sunlight strength , see .vsh for colors
#define SHADOWDISTANCE 30.0 
#define SHADOW_CLAMP 0.5
#define SHADOW_RES 256.0

/* SHADOWRES:256 */				//affect shadow map resolution (change SHADOW_RES with it!)
/* SHADOWHPL:30.0 */				//affect shadow map draw distance (change SHADOWDISTANCE with it!)
...
float ssample;
		...
		if (dist < drawdistance) {
			...
			float shadowMult = 1.0-clamp(dist-25.0,0.0,5.0)/5.0;
					if (shadowexit > 0.1) {
					shading = 1.0-shadowMult;
					}
					else
					{

						shading = (1.0-shadowMult*(clamp(comparedepth - (0.05 + (texture2D(shadow, worldposition.st + vec2(0.0,1.0/SHADOW_RES)).z) * (256.0 - 0.05)), 0.0,diffthresh)/diffthresh));
}
...
float wave = 0.0;
if (iswater > 0.9) {
wave = callwaves(worldposition.xyz*0.5)*2.0-1.0;
