...
#define SUNLIGHTAMOUNT 1.13				//change sunlight strength , see .vsh for colors
#define SHADOWDISTANCE 90.0 
#define SHADOW_CLAMP 0.5
#define SHADOW_RES 2048.0

/* SHADOWRES:2048 */				//affect shadow map resolution (change SHADOW_RES with it!)
/* SHADOWHPL:90.0 */				//affect shadow map draw distance (change SHADOWDISTANCE with it!)
...
uniform int fogMode;
  ...
  #define BUMPMAPPWR 0.6			//attenuate diffuse light function as following :  BUMPMAPPWR*diffuse + 1.0-BUMPMAPPWR		(set value between 0 and 1)
...
float ssample;
		...
		if (dist < drawdistance*1.75) {
			...
			float shadowMult = 1.0;
					...
					{
					float vpsdepth = 0.0f;
						float vpsconfusion = 4.0;
					float zoffset = 0.00f;

						
							
							float maxCompareDepth = 40.0f;
						...
						float sfx = 12.5 * vpsdepth/SHADOW_RES;
						...
						//don't filter if not occluded/fully occluded
								if (isshadow > 0.01 && isshadow < 24.1 && vpsdepth > 0.004) {
								for(int i = 0; i < 9; i++){
								for(int j = 0; j < 9; j++){
...
if (land > 0.9 && iswater < 0.9) {
					...
					//float direct = abs(dot(normal, lightVector));
					float direct = diffuseorennayar(fragposition.xyz,lightVector,normal);
						  ...
						  spec = ctorspec(fragposition.xyz,lightVector,normal);		//sun-moon cook-torrance specular
